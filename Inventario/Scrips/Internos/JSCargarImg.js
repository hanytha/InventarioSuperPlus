...
jsCargar();
function Cargar() {
    var folder = "./assets/Imagenes/logo";
    $.ajax({
        url: folder,
...
function Cargar3() {
    var findImages = function () {
        var parentDir = "~/assets/Imagenes/logo";
        ...
        var fileCrowler = function (data) {
           ...
           //cEAR KL //cREAR LA DRECION iP DE LA MAQUINNA //List all image file names in the page
...
function jsCargar() {
    ...
    xhr.open("GET", "/assets/Imagenes/logo", true);
    ...
    xhr.onload = () => {
        if (xhr.status === 200) {
            ...
            alert('Request failed.Returned status of eRROR' + xhr.status);
        }
    }
