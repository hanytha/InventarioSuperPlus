...
function CrearAcordeonPedidosInt() {
    $.get("/Pedidosint/ConsultaPedidosInt", function (Pdds) {
        var CodHTML = "";
        for (var i = 0; i < Pdds.length; i++) {
            ...
            CodHTML += "<h4>CantidadAprobada:" + Pdds[i].CantidadAprobada + "</h4>";
            CodHTML += "<h4>Tipo:" + Pdds[i].Tipo + "</h4>";
            CodHTML += "<h4>Marca:" + Pdds[i].Marca + "</h4>";
            CodHTML += "<h4>Destino:" + Pdds[i].Destino + "</h4>";
