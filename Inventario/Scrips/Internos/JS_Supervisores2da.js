...
function AcordeonSupervisores(data, IDo) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        CodHtml += "</div >";
        CodHtml += "<button class='btn btn-primary' onclick='abrirModal(" + data[i].IdSupervisor + "," + data[i].IdSupervisor + ")' data-toggle='modal' data-target='#dialogo1'><i class='fas fa-edit'></i></button> ";        CodHtml += "</div>";
        CodHtml += "</div>";
        CodHtml += "</div>";
        CodHtml += "</div>";
        CodHtml += "</div>";
